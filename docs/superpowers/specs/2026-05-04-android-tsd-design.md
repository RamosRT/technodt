...
**Проблема зависших сессий:** cookie хранится на устройстве, но сессия может стать невалидной на стороне сервера (перезапуск backend, деактивация оператора, истечение cookie). Без явной проверки приложение зависает на HomeScreen с ошибками 401 на каждом запросе.

**Новый endpoint на backend (добавить в v1.2):** `GET /api/auth/me`
```json
// 200: {"operator": "Иванов", "is_admin": false}
...
| Ситуация | Поведение |
|---|---|
| Старт приложения, cookie есть | `GET /api/auth/me` — если 200, идём на HomeScreen; если 401, очищаем cookie и на LoginScreen |
| Старт приложения, сервер недоступен | Показываем экран «Сервер недоступен» с кнопкой «Повторить» — **не выходим** из аккаунта |
| 401 в ходе работы | OkHttp `Authenticator` перехватывает, однократно пробует `GET /api/auth/me`; если снова 401 — принудительный выход на LoginScreen |
| Сервер недоступен в ходе работы | Toast «Сервер недоступен», операция не сбрасывается — **не выходим** |
| Оператор деактивирован админом | Следующий запрос вернёт 401 → принудительный выход |

**Ключевое правило:** различаем `401/403` (проблема сессии → выход) от сетевых ошибок `IOException` (проблема связи → Toast, не выход). Логаут только при явном отказе сервера в авторизации.

**Cookie lifetime:** backend должен выставлять `max-age` для cookie (например, 8 часов — рабочая смена). По истечении — автоматический 401 → выход. Это решает проблему «вечных» сессий на ТСД.
