...
payload = {
    ...
    # archive_path is resolved on Konvert-track from Paperless metadata (media_filename).
    # Optional hook-side UNC only when metadata API is unavailable:
    "archive_path": (
        onec_path_from_paperless_path(os.getenv("DOCUMENT_ARCHIVE_PATH"))
        or onec_path_from_paperless_path(os.getenv("DOCUMENT_SOURCE_PATH"))
    ),
